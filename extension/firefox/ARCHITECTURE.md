...
Version `0.1.0` reuses Parabolic's existing custom URI protocol:

```text
parabolic://example.org/video
```

The desktop application converts this back to an HTTPS URL and opens its existing add-download interface. The first button in the popup sends the page URL to `yt-dlp`; this is normally the most reliable route for supported platforms. Direct detected streams are offered as fallbacks.
